  ...
  app.post("/api/hubspot/test-connection", async (req, res) => {
    try {
      ...
      if (!activeToken) {
        return res.json({
          connected: false,
          message: "Nenhum token do HubSpot foi fornecido.",
        });
      }
    ...
    try {
      ...
      if (!ai) {
        return res.status(503).json({
          ...
          fallbackAdvice:
            "Para projetos acima de 10m em 4K HDR, use Cabos HDMI de Fibra Óptica Ativa Discabos ou Extensores HDBaseT AVLIFE. Para passagens estreitas em conduíte, use a linha de Fibra com ponta destacável.",
        });
      }
